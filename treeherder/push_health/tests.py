...
from treeherder.model.models import FailureLine, Job, OptionCollection, JobType
...
def get_test_in_progress_count(push):
    test_types = JobType.objects.exclude(name__contains="build", symbol='mozlint',)
    return (
        Job.objects.filter(push=push, tier__lte=2, result='unknown', job_type__in=test_types,)
